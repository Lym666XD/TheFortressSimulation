...
- Current item definitions are loaded from `data/core/items/*.json` through the Content-owned core catalog loader and attached to the world item manager.
- Runtime item moves, carries, additions, removals, and splits increasingly use `ItemsDiffLog` / simulation diff seams.
- Crafting, transport, construction, and stockpile systems currently rely on item IDs, tags, material IDs, GUIDs, and deterministic queue/diff behavior.
- Full equipment, damage, encumbrance, container, consumable, and save/load semantics below are target/domain design unless verified against current loaders and tests.
- The older manager/debug-spawn implementation note is archived at `../archive/simulation/CREATURE_ITEM_MANAGER_IMPLEMENTATION_2025.md`.
