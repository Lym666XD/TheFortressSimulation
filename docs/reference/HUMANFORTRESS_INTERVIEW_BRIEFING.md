...
- **Content registry is still split**: startup now uses `FortressContentLoader` and `RuntimeContentRegistryLoader` to load both legacy and structured registries, while `CoreContentCatalogLoader` loads item/creature/core-data catalogs. The long-term goal is still one ownership model with compatibility paths removed.
