...
- This is an older target design, not the exact current implementation.
- The current code has `InputBindingsService` and content-resolved input bindings, but not a fully device-agnostic `InputEvent -> InputMapper -> CommandFactory` pipeline.
- Keyboard and mouse behavior is still distributed through App runtime routers, placement controllers, and older UI component handlers.
- Keep this document as design direction until a current `INPUT_SYSTEM.md` replaces it.
