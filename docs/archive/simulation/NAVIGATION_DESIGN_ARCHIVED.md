...
Current implementation note (2026-06-12): `HumanFortress.Navigation` owns pathfinding and cache logic without a direct project dependency on `HumanFortress.Simulation`. `HumanFortress.Runtime` provides the Simulation-backed navigation source/adapter and rebuilds dirty navigation data in the runtime post-tick pipeline.
