...
- The current runtime composition is `SimulationRuntimeSystems -> UnifiedJobsOrchestrator`.
- Transport, mining, construction, and craft executor cores now live mostly in `HumanFortress.Jobs`.
- App still owns wrapper systems, concrete diff emitters, profession adapters, tunings, and orchestration wiring.
- Old references to `HaulJobSystem` are historical; current code uses `TransportJobSystem`, `TransportRequestQueue`, and `HumanFortress.Jobs.Transport.TransportJobExecutor`.
- For the current transport path, read [TRANSPORT_SYSTEM.md](../../simulation/TRANSPORT_SYSTEM.md).
...
- Job Appliers (adapters): Wrap existing executors (`TransportJobSystem`, `MiningJobSystem`, `ConstructionJobSystem`, `CraftJobSystem`) as a single “Merge+Apply job per subsystem” in v1. In v2, split per chunk.
...
- v1 adapters aggregate each subsystem’s outboxes into one Merge+Apply call per subsystem (no per-chunk split yet):
  - TransportJobSystemAdapter: drains transport requests in a stable order, performs assignment/pathing/movement, and emits DiffLog ops (MoveCreature/MarkCarried/MoveItem/UnmarkCarried).
