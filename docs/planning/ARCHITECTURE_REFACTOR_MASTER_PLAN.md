...
- The audit's direct UI live-world concern has been largely addressed by Runtime-owned snapshot/query facades and Contracts-owned read DTOs. Current App source scans show no active direct App references to Jobs, Simulation, Navigation, Core.Commands, or Runtime.Commands; remaining Runtime access in App is concentrated in App.Runtime adapters and state/session bootstrap collaborators. Movement/path ownership is still a later hardening area.
