...
- Moved the runtime host wrapper itself into `HumanFortress.Runtime` as `SimulationRuntimeHost<TSystems>`, deleting the App-owned wrapper while keeping concrete `SimulationRuntimeSystems` in App for the current adapter/logging/content hooks.
- Added an App-side `FortressRuntimeHostFactory` so `GameStateManager` no longer directly constructs the generic host or owns initial-worker/auto-dig startup hook details.
- Split concrete system construction out of `SimulationRuntimeSystems` into `FortressRuntimeSystemsFactory`, making `SimulationRuntimeSystems` a smaller App-owned collection/registration surface instead of a factory plus container.
- Split initial-worker and optional auto-dig startup hooks into `FortressRuntimeStartup`, keeping host creation separate from App-specific simulation startup seeding.
...
`GameStateManager` no longer directly creates the active `World`, navigation manager, or runtime host graph. World/session reset and navigation creation now sit behind Runtime's `SimulationRuntimeSessionFactory<SimulationRuntimeHost<SimulationRuntimeSystems>>`, with `GameStateManager` holding one runtime session handle and delegating App-specific host creation/startup hooks to `FortressRuntimeHostFactory`.
