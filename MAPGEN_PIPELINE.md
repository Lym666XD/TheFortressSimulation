...
Exactly one cavern band (performance).
...
Strata: apply home-cell geology envelope across the whole fortress map.

Aquifer: mark columns by geology flags (light/heavy).

Cavern Band ×1: one continuous cavern layer (parameters: openness, water %, flora dots).

Guarantee ≥ 2 natural entrances to surface (location seeded from surface hints).

Magma (optional): small magma pockets (no global sea).

Stamp POIs: ore clusters, ruins, shafts; write to L0/L2/L7 authoritatives.
