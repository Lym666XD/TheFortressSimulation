...
Clear passability semantics from the L0â€“L7 tile stack (terrainâ†’meta). 
TILE_LAYERS
...
Each chunk (32Ã—32) maintains derived navigation caches rebuilt during RebuildDerived (commit phase):

NavMask[idx] : byte/ushort â€?capability bits (Walk/Crawl/Swim/Fly/Standable/EdgeClimb, etc.).

NavCost[idx] : ushort â€?movement cost baseline including fluids/surface/traffic.

ConnectivityVersion : int â€?bump when topology-relevant data changes; consumers invalidate caches by version. 
DATA_LAYOUT
...
3) Passability Semantics (L0â†’L7 precedence)
When computing passability and cost, layers are consulted in this order (deterministic): L0/L2 â†?L3 â†?L4 â†?L5/L6. Hard blockers in earlier layers preempt later ones. 
TILE_LAYERS
...
bit4 Standable   // valid â€œstopâ€?tile for walkers
...
Standable is Walk plus â€œnot a ramp down edge unless coming from proper directionâ€?
...
Vertical steps use dedicated edge costs (see Â§4.3).
...
Diagonal (if enabled) = 14 (approx âˆ?*10).
...
If source and goal chunks differ in ConnectivityVersion history during planning, the service falls back to A* (v1). P1 may add per-mode RegionId labeling to early-out â€œdisconnectedâ€?queries; for now, we rely on A* with closed-set size cap.
...
Movement execution samples the next 1â€? steps from the path each tick.
...
The AI/job flow already includes â€œReplan Pathâ€?on block/timeout. 
GAME_ARCHITECTURE
...
Repeated seeds & same world state â†?identical path Hash.
...
12) Tests (must-haves)
Semantics: every L0/L2/L3 combination â†?expected NavMask/NavCost.
...
Size-aware navigation (2Ã—1 creatures), ladders/ropes as explicit edge records.

Per-actor hazard sensitivity and threat maps from the Incident Director.

Appendix A â€?Where this hooks into the engine
...
Pathfinding sits in the AI/Job flow with requestâ†’cacheâ†’A*â†’result, as diagrammed in the architecture docs. 
GAME_ARCHITECTURE
